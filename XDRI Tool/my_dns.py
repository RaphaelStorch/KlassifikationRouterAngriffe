...
class DNSStrField(StrField):
    ...
    def i2m(self, pkt, x):
        ...
        ret = b""
        for label in re.split(b"(?<!\\\\)\.", x):
            label = label.replace(b"\.", b".")
            label = label.replace(b"\\000", b"\x00")
            label = label[:63]
            ret += chb(len(label)) + label
        x = ret
...
class RDataField(StrLenField):
    ...
    def m2i(self, pkt, s):
        ...
        elif pkt.type in [2, 5, 12, 39]:  # NS, CNAME, PTR, DNAME
    ...
    def i2m(self, pkt, s):
        ...
        elif pkt.type in [2, 3, 4, 5, 12, 39]:  # NS, MD, MF, CNAME, PTR, DNAME
...
class DNSRRNAPTR(_DNSRRdummy):
    ...
    fields_desc = [DNSStrField("rrname", ""),
                   ...
                   IntField("ttl", 0),
                   ShortField("rdlen", None),
                   ShortField("order", 100),
                   ShortField("preference", 10),
                   
                   FieldLenField("flagslen", None, fmt="!B", length_of="flags"),
                   StrLenField("flags", "", length_from=lambda pkt: pkt.flagslen),
                   
                   FieldLenField("servicelen", None, fmt="!B", length_of="service"),
                   StrLenField("service", "", length_from=lambda pkt: pkt.servicelen),
                   
                   FieldLenField("regexlen", None, fmt="!B", length_of="regex"),
                   StrLenField("regex", "", length_from=lambda pkt: pkt.regexlen),
